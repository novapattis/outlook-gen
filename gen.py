...
def create():
    ...
    password = f"PattiSbaba123KacakGen"
    ...
    creationpage.send_keys(f"novapattis_{random_number}")
    os.system("color 2")
    print(Fore.RED + f"[+]Fetched outlook :novapattis_{random_number}@outlook.com"+Fore.RESET)
    ...
    os.system('cls')
    print(Fore.LIGHTMAGENTA_EX+"Robot Doğrulamasını Tamamlayın...")
    WebDriverWait(driver, 10000).until(EC.presence_of_element_located((By.ID, "KmsiCheckboxField"))).click()
    with open("accounts.txt", "a") as file:
        file.write(f"novapattis_{random_number}@outlook.com")
        os.system('color a')
        os.system('cls')
        print('Robot Doğrulama Başarılı!')
        print('Hesaplar accounts.txt Dosyasına Kaydedildi')
        time.sleep(1)
        driver.quit()
        tekrar()
